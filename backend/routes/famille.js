...
router.get('/membres-count', [auth, isAdmin], async (req, res) => {
    try {
        console.log(">>> Appel à /membres-count");

        const familles = await Famille.find().populate({
            path: 'membres.personne',
            model: 'Personne'
        });

        console.log("Familles récupérées:", familles.length);
        ...
        for (const famille of familles) {
            ...
            const membresValides = membres.filter(m =>
                m.personne && typeof m.personne === 'object' && m.personne._id
            );
